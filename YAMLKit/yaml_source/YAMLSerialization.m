...
static yaml_document_t* YAMLSerializationToDocument(id yaml, YAMLWriteOptions opt, NSError **error) {
    ...
    if (!document) {
        YAML_SET_ERROR(YAMLErrorCodeOutOfMemory, @"Couldn't allocate memory", @"Please try to free memory and retry");
        return NULL;
    }
    
    if (!yaml_document_initialize(document, NULL, NULL, NULL, 0, 0)) {
        YAML_SET_ERROR(YAMLErrorInvalidYamlObject, @"Failed to initialize yaml document", @"Underlying data structure failed to initalize");
        free(document);
        return NULL;
    }
    ...
    if ([yaml isKindOfClass:[NSDictionary class]]) {
        ...
        YAML_SET_ERROR(YAMLErrorInvalidYamlObject, @"Yaml object must be either NSDictionary of NSArray. Scalar roots are pointless.", @"Send dictionary or array as yaml document root");
        free(document);
        return NULL;
    }
...
static id YAMLSerializationWithDocument(yaml_document_t *document, YAMLReadOptions opt, NSError **error) {
    ...
    if (opt & YAMLReadOptionStringScalars) {
        // Supported
    } else {
        YAML_SET_ERROR(YAMLErrorInvalidOptions, @"Currently only YAMLReadOptionStringScalars is supported", @"Serialize with YAMLReadOptionStringScalars option");
        return nil;
    }
    ...
    for (node = document->nodes.start, i = 0; node < document->nodes.top; node++, i++) {
        switch (node->type) {
            case YAML_SCALAR_NODE:
                objects[i] = [[NSMutableString alloc] initWithUTF8String: (const char *)node->data.scalar.value];
                              ...
                              error: (NSError **) error
{
    ...
    if (!yaml_parser_initialize(&parser)) {
        YAML_SET_ERROR(YAMLErrorCodeParserInitializationFailed, @"Error in yaml_parser_initialize(&parser)", @"Internal error, please let us know about this error");
        return nil;
    }
    ...
    while (!done) {
        ...
        if (!yaml_parser_load(&parser, &document)) {
            YAML_SET_ERROR(YAMLErrorCodeParseError, @"Parse error", @"Make sure YAML file is well formed");
            return nil;
        }
             ...
             error: (NSError **) error
{
    ...
    if (!yaml_emitter_initialize(&emitter)) {
        YAML_SET_ERROR(YAMLErrorCodeEmitterError, @"Error in yaml_emitter_initialize(&emitter)", @"Internal error, please let us know about this error");
        return NO;
    }
    ...
    if (YAMLWriteOptionMultipleDocuments & opt) {
        ...
        for(id documentObject in yaml) {
            ...
            if (!document) {
                YAML_SET_ERROR(YAMLErrorInvalidYamlObject, @"Failed to de-serialize document at index %d", @"Document root can be either NSArray or NSDictionary");
